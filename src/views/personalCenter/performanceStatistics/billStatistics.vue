 ...
 * @LastEditTime: 2019-08-29 18:18:50
 -->
<template>
    <div class="page">
            ...
            <div class="bill-list">
                <div class="page-block" v-for="(item, index) of billList" :key="index">
                    <div class="block-title list-title">
                        <span class="block-title-item">{{item.billTypeName}}</span>
                        <span class="block-title-item" v-show="isShowNum"></span>
                    </div>
                    <div class="plate">{{item.plateNumberAll}}</div>
...
export default {
    ...
    methods: {
         ...
         * @LastEditTime: Do not edit
         * @return:
         * @Date: 2019-08-28 20:16:39
         */
        selectMonthHandle(date) {
            this.canLoad = true;
            this.currentDate = date;
            this.page = 1;
            this.getBillStatisticsList(date);
        },
        //获取用户ID值
        getUserId() {
            let params = this.$session.get("dateSearchKey");
        ...
        getBillStatisticsList(date) {
            ...
            billApi.getBillStatisticsList(params).then(res => {
                /*  this.$toast.loading({
                    duration: 0
                });
                this.loadText = "加载中..."; */
         ...
         */
        getSortRuleInfo() {
            billApi.getSortRuleInfo().then(res => {
                if (res.code == 20000) {
        ...
        scrollWatch() {
            window.onscroll = () => {
                ...
                if (a + b == c) {
                    // 已滚动到底部
                    this.page++;
                    this.getBillStatisticsList(this.currentDate);
                }
            };
        },
        // 打开筛选面板
